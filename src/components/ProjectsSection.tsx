      ...
      <section className="min-h-screen py-16 md:py-24 px-4 md:px-8">
        <div className="w-full max-w-[720px] mx-auto">
          ...
          <div className="flex items-center justify-center gap-4 md:gap-6 mb-12 md:mb-20">
            ...
            <h2 className="text-[28px] md:text-[36px] font-['Solway'] text-[#474747] whitespace-nowrap">Projects</h2>
            <div className="h-[2px] w-[40px] bg-[#474747] rounded-full"></div>
          </div>
          ...
          {loading && (
            <div className="text-center py-8">
              <p className="font-['Gaegu'] text-[18px] text-[#8c8fa6]">Loading projects...</p>
            </div>
          )}
          ...
          {!loading && projects.length === 0 && (
            <div className="text-center py-8">
              <p className="font-['Gaegu'] text-[18px] text-[#8c8fa6]">No projects yet. Check back soon!</p>
            </div>
          )}
          ...
          <div className="min-h-screen py-8 px-4" onClick={(e) => e.stopPropagation()}>
            <div className="max-w-3xl mx-auto">
              <div className="flex items-center justify-between mb-6">
                <div className="font-['Gaegu'] text-[14px] text-[#8c8fa6]">Projects / {selectedProject.title}</div>
                <button
                  onClick={() => setSelectedProject(null)}
                  className="p-2 hover:bg-gray-100 rounded-full transition-colors"
                >
                  <X className="w-5 h-5 text-[#474747]" />
                </button>
              </div>

              <h1 className="text-[36px] md:text-[48px] font-['Solway'] font-bold text-[#474747] mb-4 leading-[1.2]">{selectedProject.title}</h1>
              <p className="font-['Gaegu'] text-[18px] text-[#8c8fa6] mb-12 leading-[1.6]">{selectedProject.description}</p>
              ...
              {selectedProject.tags && selectedProject.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mb-12">
                  {selectedProject.tags.map((tag, i) => (
                    <span
                      key={i}
                      className="px-3 py-2 bg-[#f8f9fc] border-2 border-[#e5e7f0] rounded-lg text-[14px] font-['Gaegu'] text-[#474747]"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="mt-16 md:mt-20 pt-6 border-t border-[#474747]">
                <button
                  onClick={() => setSelectedProject(null)}
                  className="font-['Gaegu'] text-[18px] text-[#474747] hover:underline"
                >
